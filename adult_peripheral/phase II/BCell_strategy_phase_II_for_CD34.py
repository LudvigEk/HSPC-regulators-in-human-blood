...
def gateBCellDataSet(fcs, *args, **kwargs):
    if not 'manual_CD34_lim' in kwargs:
        raise ag.AliGaterError("in strategy: ","manual CD34 limits not in argument")
    ...
    #CD34pos
    #CD34pos xlim, for manual adjustments
    manual_xlim=kwargs['manual_CD34_lim']
    reportStr="Manually specified CD34 xlim: "+str(manual_xlim)+"\n"
    sys.stderr.write(reportStr)
    CD34step1=ag.gateTiltedLine(fcs,xCol="CD34",yCol="CD45",name="CD34step1",parentGate=PBMC,startPoint=(manual_xlim, ylim),theta=60, endLimits=(None,2000), population='lower',scale='bilog', T=200, filePlot=None)
    CD34step2=ag.gateTiltedLine(fcs, xCol="CD34",yCol="CD45",name="CD34step2",parentGate=CD34step1,startPoint=(manual_xlim, ylim),theta=-60, endLimits=(None,200), population='upper', scale='bilog', T=200, filePlot=None)
    CD34pos=ag.gateThreshold(fcs,name="CD34pos",xCol="CD34",yCol="CD45",parentGate=CD34step2,thresh=50000,population='lower',scale='bilog', T=200,update=False)
...
if __name__ == '__main__':
    ...
    index_file_path="/media/ludvig/Project_Storage/BloodVariome/aligater_output/phase_II_CD34_reruns/"+str(CD34limit)+"_rerun_index.txt"
    index_file = ag.pd.read_csv(index_file_path, sep='\t',header=None)[0].tolist()
    exclusions=[] #ag.pd.read_csv("/media/ludvig/cbio3/projects/GAIM/metadata/Phase_II/quality_control/BCell_panel/BCell_exclusion.txt", sep="\t", header=None)[0].tolist()
    ...
    BCell_exp.apply(gateBCellDataSet, folder='phase_II/tmp', manual_CD34_lim=CD34limit)
    resultfileStr="/media/ludvig/Project_Storage/BloodVariome/aligater_output/phase_II_CD34_reruns/phase_II_BCell_phase_II_06-12-2018_rerun_CD34_all_last_"+str(CD34limit)+".txt"
    BCell_exp.printExperiment(resultfileStr)
